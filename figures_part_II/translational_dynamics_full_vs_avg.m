...
set(gcf, 'Position',  [1, 600, 1150, 450])
tl=tiledlayout(1,2);
...
str = streamribbon(vertices, twistangle ,1);
...
camlight(0,0)
axis equal
col = colormap(parula(256));
colormap(col)
c=colorbar;

c.Position = [0.42 0.2 0.015 0.7];
c.AxisLocation = 'in';
c.Ticks = [-2.35 -1.57 -0.79];
c.TickLabels = {'$-\frac{3\pi}{4}$','$-\frac{\pi}{2}$','$-\frac{\pi}{4}$'};
c.TickLength = 0.02;
c.TickLabelInterpreter = 'latex';
c.Label.String='$ \psi $';
c.Label.Interpreter='latex';
c.Label.Rotation=0;
c.Label.Position=[0.5 -0.35];
c.Label.FontSize=24;
...
% zooom on a small interval.

nexttile(2);
% Plot the average trajectory.
tmin = 4.8e4;tmax=5e4;
tick=10; %trick to make the line look smoother
plot3(x_bar(tmin:tick:tmax),y_bar(tmin:tick:tmax),z_bar(tmin:tick:tmax),'r','LineWidth',4);
hold on

% Plot the full trajectory.
plot3(x_full(tmin:tick:tmax),y_full(tmin:tick:tmax),z_full(tmin:tick:tmax),'k','LineWidth',4);

% Ribbon plot representing the psi angle.
vertices = {[x_full(tmin:tmax), y_full(tmin:tmax), z_full(tmin:tmax)]};
psi_mod = mod(psi_full(tmin:tmax), 2*pi) - pi;
twistangle = {[psi_mod(1);diff(psi_mod)]};
str = streamribbon(vertices, twistangle ,0.03);
...
exportgraphics(gcf,'translational_dynamics_full_vs_avg.png','Resolution',300)
